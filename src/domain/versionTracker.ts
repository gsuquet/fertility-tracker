...
export const VERSION_HISTORY: VersionRelease[] = [
  ...
  },
  {
    ...
    titleFr: 'Qualité de Code, Analyse Statique, Couverture V8 & Pipeline CI/CD',
    tagline:
      'Configured Biome linter, Prettier code formatter, strict TypeScript compiler flags, Vitest V8 code coverage, and CI/CD quality gates',
    taglineFr:
      'Configuration du linter Biome, formateur Prettier, options strictes TypeScript, couverture de tests Vitest V8 et pipeline CI/CD',
    crmsSpecVersion: '1.1.0',
    highlights: [
      'Integrated Biome linter and Prettier formatter across the entire repository with automated npm scripts',
      'Enabled strict TypeScript unused variable checking (noUnusedLocals, noUnusedParameters) and cleaned dead code',
      'Configured Vitest V8 coverage engine with detailed reporting across domain algorithms, components, and contexts',
      'Enhanced GitHub Actions CI workflow with 5-stage quality gates: dependencies, linting, formatting, coverage, and production build verification',
    ],
    highlightsFr: [
      "Intégration du linter Biome et du formateur Prettier sur l'ensemble du dépôt avec scripts npm dédiés",
      'Activation des vérifications strictes TypeScript pour les variables inutilisées (noUnusedLocals, noUnusedParameters) et nettoyage du code mort',
      'Configuration du moteur de couverture Vitest V8 avec rapports détaillés sur les algorithmes, composants et contextes',
      'Amélioration du pipeline GitHub Actions CI avec 5 étapes de contrôle : dépendances, linting, formatage, couverture et build de production',
    ],
  },
  {
    version: '1.9.2',
    ...
    tagline:
      'Standardized cycle picker height to 38px and restructured mobile footer into clean rows with bottom-nav clearance',
    taglineFr:
      'Standardisation de la hauteur du sélecteur de cycle à 38px et restructuration du pied de page mobile avec dégagement de la barre inférieure',
    crmsSpecVersion: '1.1.0',
    highlights: [
  ...
  },
  {
    ...
    tagline:
      'Portaled mobile action sheet directly to document.body to prevent ancestor backdrop-filter displacement',
    taglineFr:
      "Rendu du volet d'actions mobile via un portail sur document.body pour éviter les décalages liés au backdrop-filter",
    crmsSpecVersion: '1.1.0',
    highlights: [
  ...
  },
  {
    ...
    tagline:
      'Consolidated mobile header into a sleek More menu and transformed all dialogs into responsive native bottom sheets',
    taglineFr:
      "Regroupement des actions de l'en-tête mobile dans un menu Plus et conversion des modales en volets inférieurs adaptés",
    crmsSpecVersion: '1.1.0',
    highlights: [
  ...
  },
  {
    ...
    tagline:
      'Fixed dark mode styling for detailed selectors toggle, PDF export configuration card, and modal action buttons',
    taglineFr:
      "Correction du style en mode sombre pour le bouton des sélecteurs détaillés, la configuration d'export PDF et les boutons de modales",
    crmsSpecVersion: '1.1.0',
    highlights: [
  ...
  },
  {
    ...
    tagline:
      'Collapsed brand text on mobile viewports so only the logo is displayed, preventing header bar overflow',
    taglineFr:
      "Masquage du texte de la marque sur mobile pour n'afficher que le logo, évitant le débordement de l'en-tête",
    crmsSpecVersion: '1.1.0',
    highlights: [
  ...
  },
  {
    ...
    tagline:
      'Modularized monolithic styling with native CSS @layer, formalized design tokens, and domain stylesheets (ADR 0002)',
    taglineFr:
      'Modularisation des styles avec CSS @layer natif, jetons de design formalisés et feuilles de style dédiées (ADR 0002)',
    crmsSpecVersion: '1.1.0',
    highlights: [
  ...
  },
  {
    ...
    tagline:
      'Removed whitespace between mucus stretch/modifier and frequency in both code formatting and parsing (e.g. 10KLX3, 0AD)',
    taglineFr:
      "Suppression des espaces entre la glaire et la fréquence dans le formatage et l'analyse des codes (ex. 10KLX3, 0AD)",
    crmsSpecVersion: '1.1.0',
    highlights: [
  ...
  },
  {
    ...
    tagline:
      'Centered modal layout overlaying top of page with elevated z-index for optimal visibility across all screen sizes',
    taglineFr:
      'Affichage centré de la modale superposée avec z-index élevé pour une visibilité optimale sur tous les écrans',
  ...
  },
  {
    ...
    tagline:
      'Interactive prompt and Observation Drawer toggle to designate whether the first bleeding day of a series marks a new cycle boundary',
    taglineFr:
      'Invitation interactive et commutateur dans le tiroir pour désigner si le premier jour de saignement marque un nouveau cycle',
    crmsSpecVersion: '1.0.0',
    highlights: [
  ...
  },
  {
    ...
    tagline:
      'Distinct current day visual styling across Calendar and Chart views with glowing accent borders, circular date badges, today headers, and screen reader accessibility',
    taglineFr:
      "Mise en valeur visuelle du jour actuel sur le calendrier et le graphique avec bordures lumineuses, timbres circulaires et compatibilité lecteurs d'écran",
    crmsSpecVersion: '1.0.0',
    highlights: [
  ...
  },
  {
    ...
    tagline:
      'Mobile top stats removal on graph & calendar, 5-day aligned history items, streamlined version history accordion, and standardized lowercase dash-separated export filenames',
    taglineFr:
      "Masquage des stats mobiles sur graphique et calendrier, alignement de l'historique 5 jours, accordéon de version épuré et noms d'exportation en minuscules avec tirets",
    crmsSpecVersion: '1.0.0',
    highlights: [
  ...
  },
  {
    ...
    tagline:
      'Compact PDF chart export grid with 2-line observation code wrapping, dedicated free-form notes row, auto-close modal flow, and symptom code formatting',
    taglineFr:
      'Grille PDF compacte avec repli du code sur 2 lignes, ligne de notes libres en-dessous, fermeture automatique du modal et séparation des symptômes',
    crmsSpecVersion: '1.0.0',
    highlights: [
  ...
  },
  {
    ...
    tagline:
      'Multi-step interactive onboarding, CrMS biomarker education, dual-mode logging guide, demo data loader, and persistent first-visit tracking',
    taglineFr:
      "Guide d'accueil interactif, tutoriel biomarqueurs CrMS, guidage de saisie double-mode et données démo en un clic",
    crmsSpecVersion: '1.0.0',
    highlights: [
  ...
  },
  {
    ...
    taglineFr:
      'Système de suivi des versions intégré, optimisations de rendu et barre supérieure mobile adaptative',
    crmsSpecVersion: '1.0.0',
    highlights: [
...
export function checkAndRecordVersionSeen(): {
